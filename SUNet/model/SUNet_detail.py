...
# Dual up-sample
class UpSample(nn.Module): # elso retege a decoder-nek
    ...
    def forward(self, x):
        ...
        B, L, C = x.shape # torch.Size([1, 64, 768])
        x = x.view(B, H, W, C)  # B, H, W, C
        x = x.permute(0, 3, 1, 2)  # B, C, H, W
        # ezelott kihajtogatjuk az x-et -> torch.Size([1, 768, 8, 8])
        x_p = self.up_p(x)  # pixel shuffle
        # x_p.shape -> torch.Size([1, 384, 16, 16])
        x_b = self.up_b(x)  # bilinear
        # x_b -> torch.Size([1, 384, 16, 16])
        out = self.conv(torch.cat([x_p, x_b], dim=1))
        # self.conv utan, torch.Size([1, 384, 16, 16]), self.conv visszapreseli a dimenziot ugyanakkorara
        out = out.permute(0, 2, 3, 1)  # B, H, W, C
        if self.factor == 2:
            out = out.view(B, -1, C // 2) # csak a felezes miatt van a C//2. Igazabol nem valtoztat a csatornaszamon

        return out # torch.Size([1, 256, 384])
...
class SUNet(nn.Module):
    ...
    def forward_features(self, x):
        residual = x
        x = self.patch_embed(x) # 4x4-es patch ek lesznek, amiket reprezentalunk egy 96 dimenzios vektorral es kilapitjuk patch ek menten (256/4)^2 darab patch lesz
        if self.ape:
            x = x + self.absolute_pos_embed # ez kimarad
        x = self.pos_drop(x)
        x_downsample = []

        for layer in self.layers:
            x_downsample.append(x)
            x = layer(x) # minden layer egy csomo SWINN-transzformer, ami utan downsample-öl minden 2x2-es blokkot es a csatornaszam 2x-es lesz

        x = self.norm(x)  # B L C # 4096 -> 64 lett a csatornaszam vagyis 3x lett 1/4-edelve a meret x.shape = torch.Size([1, 64, 768]) -> 8x8-as kep, amin minden csatorna 768 = 8*96 = 2^3*96 (egy BasicLayer nem downscalel)
    ...
    def forward(self, x):
        x = self.conv_first(x) # tartja a meretet, felfujja 96 dim re a 3 csatornat
        x, residual, x_downsample = self.forward_features(x)
        x = self.forward_up_features(x, x_downsample)
        x = self.up_x4(x) # x.shape elotte: torch.Size([1, 4096, 96]), sima upsampling van a belsejeben
        out = self.output(x) # torch.Size([1, 96, 256, 256]) # a csatorna beken van hagyva, 64x64-es kepbol lett 256x256-os
        # x = x + residual
        # self.output -> sima Linear reteg, amiben a kimeneti csatornaszam a SUNET ctor parametereben jon meg. Jelen feladatban 3
        # mert denoising-olunk
