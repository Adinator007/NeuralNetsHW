...
from SSIM import SSIM, SSIMLoss, MSSSIMLoss
...
def plot_ae_outputs_den(device, net, n=10,noise_factor=0.3, dataloader = None):
    net = net.to(device)
...
if __name__ == '__main__':
    ...
    net = torch.load("D:/InfoAnyagok/ArtificialIntelligence/Vision/NeuralNets/TrainedModels/SUNet/MSSSIM.pth")
    haveCuda = False
    if haveCuda:
        net = net.cuda()
    ...
    scheduler = lr_scheduler.CosineAnnealingLR(optimizer,
                                    numEpoch // 2)  # mikor csokkenti -> az epoch ok felenek eltelte utan, es a felere csokkenti a lr t
    ...
    scheduler = lr_scheduler.StepLR(optim, numEpoch // 2)

    train_loss = []

    train_loss = []

    device = torch.device("cuda") if haveCuda else torch.device("cpu")
    ...
    for epoch in range(numEpoch):
        print('EPOCH %d/%d' % (epoch + 1, numEpoch))
        train_loss = 0.0
        for image_batch in tqdm(traindataloader): # TODO change
        ...
        print('\n EPOCH {}/{} \t train loss {}'.format(epoch + 1, numEpoch, train_loss / len(traindataloader)))
        sum_val_loss = 0.0
        ...
        print('\n EPOCH {}/{} \t val loss {}'.format(epoch + 1, numEpoch, sum_val_loss / len(testdataloader)))
