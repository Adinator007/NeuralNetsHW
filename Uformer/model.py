...
from SSIM import msssim, ssim, SSIMLoss, MSSSIMLoss
...
#########################################
########### LeWinTransformer #############
class LeWinTransformerBlock(nn.Module): # same as SformerBlock in file:///C:/Users/trana/Downloads/electronics-11-00418-v2.pdf
...
def main(b):
    ...
    '''net = arch(img_size=input_size, embed_dim=8, depths=depths,
    ...
    net = torch.load("D:/InfoAnyagok/ArtificialIntelligence/Vision/NeuralNets/TrainedModels/UFormer/MSSSIM.pth")
    haveCuda = False
    # haveCuda = False
    if haveCuda:
        net = net.cuda()
    ...
    scheduler = lr_scheduler.CosineAnnealingLR(optim, num_epochs // 2)
    ...
    traindataloader = torch.utils.data.DataLoader(
        ListDataset(
            # r'D:/InfoAnyagok/ArtificialIntelligence/Vision/NeuralNets/SUNet/datasets/archive/img_align_celeba/img_align_celeba/',
            # r'/home/adam/TokenLearner_Object_Detection/pythonProject/img_align_celeba/',
            r'/root/workdir/datasets/celeba/img_align_celeba/',
            img_size=(256, 256), test=False), batch_size=8,
        shuffle=True)
    ...
    testdataloader = torch.utils.data.DataLoader(
        ListDataset(
            ...
            # r'/home/adam/TokenLearner_Object_Detection/pythonProject/img_align_celeba/',
            r'/root/workdir/datasets/celeba/img_align_celeba/',
            img_size=(256, 256), test=True), batch_size=8,
        shuffle=True)
    ...
    for epoch in range(num_epochs):
        ...
        for image_batch in tqdm(traindataloader):
            ...
            optim.step()
        print('\n EPOCH {}/{} \t train loss {}'.format(epoch + 1, num_epochs, train_loss / len(traindataloader)))
        sum_val_loss = 0.0
        ...
        if sum_val_loss / len(testdataloader) < bestLoss:
            ...
            if b:
                torch.save(net, "bestmodel1.pth")
            else:
                torch.save(net, "bestmodel2.pth")
        print('\n EPOCH {}/{} \t val loss {}'.format(epoch + 1, num_epochs, sum_val_loss / len(testdataloader)))
        scheduler.step()
    plot_ae_outputs_den(device, net, n=10, noise_factor=0.3)

    # print("number of GFLOPs: %.2f G"%(net.flops() / 1e9))
